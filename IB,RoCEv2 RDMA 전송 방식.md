...
InfiniBand와 RoCEv2는 모두 RDMA를 지원하지만 네트워크를 구성하는 방식에는 차이가 있다.

- **InfiniBand**는 RDMA를 위해 설계된 전용 네트워크로, RDMA 패킷을 그대로 전송한다.
- **RoCEv2**는 동일한 RDMA Transport를 UDP/IP로 캡슐화하여 Ethernet 네트워크에서 RDMA를 사용할 수 있도록 확장한 기술이다.
- RoCEv2가 UDP를 사용하는 이유는 UDP의 신뢰성을 활용하기 위해서가 아니라, RDMA가 이미 신뢰성을 제공하기 때문이다.
- UDP는 IP Routing과 ECMP를 지원하기 위한 캡슐화 계층 역할을 수행하며, 이를 통해 대규모 AI GPU Cluster에서도 Ethernet 기반 RDMA 환경을 구축할 수 있다.
